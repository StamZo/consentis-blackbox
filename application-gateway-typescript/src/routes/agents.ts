...
import {
  ...
  getW3cCredential,listW3cCredentials,ensurePublicFabricDid,
  getLatestBlsDid,
  connectToPublicDid
} from '../acapy';
...
//     // 1) Issuer: ensure BLS signing DID (do NOT set public)
//     let bbsDid: string | undefined;
//     let bbsVerkey: string | undefined;
//     if (isIssuer) {
//       const b = await ensureDidKey(req, peer, { forceNew, key_type: 'bls12381g2', setPublic: false });
//       bbsDid = b.did;
//       bbsVerkey = b.verkey;
//     }

//     // 2) Issuer: ensure did:fabric is public (ed25519)
//     //    Non-issuer: you can just ensure did:key ed25519 public
//     let publicDid: string;
//     let publicVerkey: string | undefined;

//     if (isIssuer) {
//       const didFabric = process.env.ISSUER_FABRIC_DID || 'did:fabric:issuer';
//       const pub = await ensurePublicFabricDid(req, peer, didFabric);
//       publicDid = pub.did;
//       publicVerkey = pub.verkey;
//     } else {
//       const pub = await ensureDidKey(req, peer, { forceNew, key_type: 'ed25519', setPublic: true });
//       publicDid = pub.did;
//       publicVerkey = pub.verkey;
//     }
...
//         process.env.ISSUER_DIDCOMM_ENDPOINT || 'http://issuer:8050'; // must be reachable from holder container

//       const headers = { ...authHdr, 'x-selected-peer': String(peer) };

//       const { data } = await axios.post(
//         `${selfBase}/storeDIDkey/${encodeURIComponent(publicDid)}`,
//         {
//           publicKeyBase58: publicVerkey,
//           serviceEndpoint,
//           bbsPublicKeyBase58: bbsVerkey || null,
//         },
//         { headers }
//       );
...
router.post('/did/bootstrap', async (req, res, next) => {
  try {
    ...
    // 1) Everyone: ensure BLS signing DID (do NOT set public)
    let bbsDid: string | undefined;
    let bbsVerkey: string | undefined;
    {
      const b = await ensureDidKey(req, peer, {
        forceNew,
        key_type: 'bls12381g2',
        setPublic: false,
      });
    ...
    // 2) Issuer: ensure did:fabric is public (ed25519)
    //    Non-issuer: ensure did:key ed25519 (NOT public)
    let publicDid: string;
    let publicVerkey: string | undefined;
    ...
    if (isIssuer) {
      ...
      const pub = await ensureDidKey(req, peer, {
        forceNew,
        key_type: 'ed25519',
        setPublic: false, // <-- changed from true
      });
      publicDid = pub.did;
      publicVerkey = pub.verkey;
    }
    ...
    res.json({
      ...
      publicDid,
      publicVerkey,
      bbsDid: bbsDid || null,     // now non-issuer also gets this
      stored,
      storeResult,
    });
  } catch (e) { next(e); }
...
router.post('/connect-to-issuer', async (req: any, res, next) => {
  try {
    ...
    res.json({ peer, connection_id: result?.connection_id, result });
  } catch (e) { next(e); }
});
...
// Optional: ?id=... to delete one, ?alias=... to delete by alias, &state=... to filter,
// no params -> delete all.
router.delete('/connections', async (req: any, res, next) => {
  try {
    const { peer } = ctx(req);
    const id    = req.query.id as string | undefined;
    const alias = (req.query.alias || req.query.contact_alias) as string | undefined;
    const state = req.query.state as string | undefined;

    if (id) {
      await deleteConnectionById(req, peer, id);
      return res.json({ peer, deleted: [id], count: 1 });
    }

    if (alias) {
      const result = await deleteConnectionsByAlias(req, peer, alias, state ? { state } : undefined);
      return res.json({ peer, deleted: result.deleted, count: result.count });
    }

    // list and filter locally so we can delete all when alias is omitted
    const conns = await listConnections(req, peer);
    const filtered = conns.filter((c: any) =>
      (!alias || c?.alias === alias) &&
      (!state || c?.state === state)
    );

    for (const c of filtered) {
      await deleteConnectionById(req, peer, c.connection_id);
    }

    return res.json({ peer, deleted: filtered.map((c: any) => c.connection_id), count: filtered.length });
  } catch (e) { next(e); }
});
...
router.post('/credentials/propose', async (req: any, res, next) => {
  try {
    ...
    //let holderDid = await getPublicDid(req, peer);
    //if (!holderDid) holderDid = await ensureDidKey(req, peer, { key_type: 'bls12381g2' });
    let holderDid = await getLatestBlsDid(c);

    if (!holderDid) {
      const result = await ensureDidKey(req, peer, { key_type: "bls12381g2" });

      holderDid = typeof result === "string"
        ? result
        : result?.did;

      if (!holderDid) {
        throw new Error("ensureDidKey returned no did");
      }
    }
...
router.post('/credentials/issue', async (req: any, res, next) => {
  try {
   ...
   const connection_id = await connectionIdFromAlias(c, alias);
    ...
    const wantsBbs = proofType === 'BbsBlsSignature2020';

    const issuer =
      issuerDid ||
      (wantsBbs
        ? await ensureDidKey(req, peer, { key_type: 'bls12381g2', /* setPublic: false */ })
        : (await getPublicDid(req, peer)) || await ensureDidKey(req, peer, { key_type: 'ed25519', /* setPublic: true */ })
      );
...
// Verification method used by the issuer to sign the VC inside the VP (if present)
function extractIssuerVerificationMethod(rec: any): string | null {
  const vc =
    rec?.by_format?.pres?.dif?.verifiablePresentation?.verifiableCredential?.[0] ??
    rec?.by_format?.pres?.dif?.presentation?.verifiableCredential?.[0] ??
    rec?.pres?.['presentations~attach']?.[0]?.data?.json?.verifiableCredential?.[0] ??
    rec?.presentation?.['presentations~attach']?.[0]?.data?.json?.verifiableCredential?.[0];
  const vm = vc?.proof?.verificationMethod;
  return typeof vm === 'string' && vm.length ? vm : null;
}
...
router.post('/proofs/verify', async (req: any, res, next) => {
  try {
    ...
    // Optional key binding: ensure the VM used in the VC matches one of the on-chain keys
    const vmId = extractIssuerVerificationMethod(matched);
    ...
    // Key check: VM must point to one of the issuer's known keys
    if (vmId) {
      const expected = [
        `${issuerDid}#keys-1`,
        `${issuerDid}#bbs-1`
      ];
      if (!expected.includes(vmId)) {
        return res.status(409).json({
          error: 'issuer_key_mismatch',
          message: `Verification method ${vmId} is not one of the on-chain keys for ${issuerDid}`
        });
      }
    }
